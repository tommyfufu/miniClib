...
{
	struct sigaction oldhander, newhandler;
	if (handler == SIG_ERR || (signum < 1) || (signum > 29))
		return SIG_ERR;
	else
	{
		...
		if (sigaction(signum, &newhandler, &oldhander) < 0)
			return SIG_ERR;
		return oldhander.sa_handler;
	}
}
...
// initialises the signal set pointed to by set, such that all signals defined in this document are excluded/included.

/**my memset for sigemptyset and sigfillset***/
...
int sigfillset(sigset_t *set) // initialize to contain all signals
{
	if (set == NULL)
		return -1;
	memset(set, 0xff, sizeof(sigset_t)); // unsigned long sig[1], 4 bytes
	return 0;
}
// adds/deletes the individual signal specified by the signo to the signal set pointed to by set.
int sigaddset(sigset_t *set, int signum) // add a signal into a set
{
	if (set == NULL || (signum < 1) || (signum > 29))
		return -1;
...
// tests whether the signal specified by signo is a member of the set pointed to by set.
// return 1 is yes, 0 is no, -1 is error
